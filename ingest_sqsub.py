...
分值的实话：外省原册**没标分值**。库里写的分数是照资中真题的口径给的
（案例 12 分、公文 15 分），简答论述资中根本没考过、连口径都没有，按 10 分记。
...
def promote_points(text):
    ...
    是为资中真题的参考答案写的、没错；但外省这批答案顶层就是用「(1)(2)(3)」编的，
...
SOURCES = [
    {"key": "多省主观题真题",
     ...
     "note": "外省真题，与资中「案例分析 + 公文写作」同型；简答论述资中未考过。"},
]
...
# 分不出来就写「社会工作」——**不硬塞进「资中县情」**，外省题里没有资中。
