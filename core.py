...
# ---------------------------------------------------------------- 板块结构
# 社区专职工作者的考点大类。**解析器（ingest_shequ.py）贴的 qtype 就是这几个词**，
# 两边必须是同一套 —— 分成两套的话，「按考点练」和「按考点统计」会各说各话，
# 而且不会报错，只会永远查不到题。
SQ_BOARDS = ["社区知识", "社会工作", "法律法规", "党建党务",
             "公文写作", "应急安全", "资中县情", "时政理论", "公基常识"]
...
SECTIONS = [
    ...
    {"key": "shenlun", "name": "申论", "icon": "申", "desc": "申论写作",
     "boards": ["应用文", "议论文"]},
    # 第三条备考线：内江市资中县社区专职工作者。和上面两条并存而不是另起一个应用，
    # 图的是整条「练 → 错题 → 复习 → 统计」的链路直接复用；两条线之间靠
    # 「备考方向」开关分开统计口径（见 mods/shequ.py），数据本身不隔离。
    {"key": "shequ", "name": "社区", "icon": "社", "desc": "社区专职工作者",
     "boards": SQ_BOARDS},
]
