#!/usr/bin/env bash
# 桌面版朗读引擎（不装就没有朗读；应用照常启动，只是朗读会提示你跑这个脚本）。
#
# 为什么要装：WebKitGTK 没有 speechSynthesis，桌面版只能借系统 TTS。
# 而系统自带的 speech-dispatcher **会段错误**（spd_pulse.so 空指针，内核日志实锤），
# 已经从应用里彻底移除 —— 所以这两个不是「锦上添花」，是**桌面版能不能朗读的前提**。
# 装完在「账户 → 🔊 朗读音色」里切：
