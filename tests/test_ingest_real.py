...
# ---- needs_asset：这题脱离图/材料还能不能做 --------------------------------

def test_定义判断不该被根据上述误锁():
    """「根据上述定义，下列属于…的是」—— 定义就写在题干里，是能做的。

    原先 _ASSET_RE 里写的是光秃秃的「根据上述」，把定义判断整类误伤（实测 154 道）。
    这个模式本来是给资料分析的，后面必须跟「资料/材料/表」。
    """
    stem = "情绪智力是指个体察觉、处理自我及他人情绪的能力。根据上述定义，下列属于情绪智力的是："
    opts = ["考试过度紧张后自我调节", "遇到恶狗吓出冷汗", "了解原委后释怀", "被指责仍克制回应"]
    assert I.needs_asset(stem, False, "判断推理", opts) == 0
    # 真的指向资料的那种照样要锁
    assert I.needs_asset("根据上述资料，下列说法正确的是：", False, "判断推理", opts) == 1


def test_通用题干但选项自带内容_不该锁():
    """is_generic 是**判重**用的信号，不是「缺资产」信号。

    「关于生活常识，下列说法错误的是」题干确实通用（配过好几组选项），
    但四个选项本身就是完整内容，这题能做（实测误锁 322 道）。
    """
    stem = "关于生活常识，下列说法错误的是："
    opts = ["腹部核磁共振时憋气可让脏器显影更充分", "食用蜡可以防止微生物侵害果实",
            "马铃薯切开后浸泡清水可防氧化", "可通过虫子取食痕迹判断蘑菇是否有毒"]
    assert I.needs_asset(stem, True, "常识判断", opts) == 0


def test_选项退化成占位符才是缺图的可靠信号():
    """题目本体在图里时，四个选项会退化成 A/B/C/D —— 那才该锁。"""
    stem = "左边给定的是正方体的外表面展开图，下面哪一项能由它折叠而成？"
    assert I.needs_asset(stem, False, "判断推理", ["A", "B", "C", "D"]) == 1
    assert I.needs_asset("下面哪一项能由它折叠而成？", False, "判断推理",
                         ["①", "②", "③", "④"]) == 1


def test_占位符判据不能只看长度():
    """数量关系的选项「18/19/20/21」也很短，那是真内容，不是占位符。"""
    stem = "甲的年龄是乙的两倍，五年后甲乙年龄之和为四十，问甲今年多少岁？"
    assert I.needs_asset(stem, False, "数量关系", ["15", "18", "20", "22"]) == 0


def test_组合选项不是占位符():
    """「①③④」说明题干里有①②③④四条表述，是能做的 —— 不能当占位符锁掉。"""
    stem = "关于人民民主，下列表述正确的是：①人民代表大会制度是根本政治制度②…③…④…"
    assert I.needs_asset(stem, False, "常识判断", ["①③④", "①②④", "①②③", "②③④"]) == 0


def test_短题干指代文段才锁_长题干自带文段不锁():
    """选词填空的题干本身就包含整段文字，里面出现「文中」是正常的，不是缺料。"""
    assert I.needs_asset("填入文中画横线部分最恰当的一项是：", False,
                         "言语理解与表达", ["甲", "乙", "丙", "丁"]) == 1
    assert I.needs_asset("这篇文章接下来最可能谈论：", False,
                         "言语理解与表达", ["保护举措", "常用工具", "文化记忆", "国际比较"]) == 1
    长题干 = ("构建新发展格局的关键在于经济循环的畅通无阻，就像人们讲的要调理好统摄全身"
              "阴阳气血的任督二脉。经济活动需要各种生产要素的组合，文中所述的循环一旦受阻，"
              "整个链条都会受到影响。填入画横线部分最恰当的一项是：")
    assert I.needs_asset(长题干, False, "言语理解与表达",
                         ["行之有效", "环环相扣", "畅通无阻", "井然有序"]) == 0


def test_作者二字不能误伤常识题():
    """「《荷塘月色》的作者是」问的是书的作者，不是文段作者。"""
    assert I.needs_asset("《荷塘月色》的作者是（ ）。", False, "常识判断",
                         ["郁达夫", "朱自清", "鲁迅", "郭沫若"]) == 0
    assert I.needs_asset("作者接下来最可能讲述的是：", False, "言语理解与表达",
                         ["甲说", "乙说", "丙说", "丁说"]) == 1


def test_asset_flag_规则只许解锁不许上锁():
    """「这题需不需要资产」归规则管，「资产够不够」归 ingest_figs/ingest_material 管。

    规则说不需要 → 直接放行；规则说需要 → 沿用资产脚本上次的裁决。
    两边各写一份「资产够不够」的标准迟早打架。
    """
    import json as _j
    能做的 = {"stem": "《荷塘月色》的作者是（ ）。", "qhash": "h1", "module": "常识判断",
              "options": _j.dumps(["郁达夫", "朱自清", "鲁迅", "郭沫若"])}
    assert I._asset_flag(能做的, set(), 1) == 0, "规则说不需要资产，就该解锁"
    图形题 = {"stem": "下面哪一项能由它折叠而成？", "qhash": "h2", "module": "判断推理",
