...
let ME = null, SECTIONS = [], IDIOM_BOARD = '';
let stack = [];                 // 导航栈
let cur = { section: '', board: '' };

/* ---------------- 导航 ---------------- */
const VIEWS = ['home', 'section', 'board', 'viewer'];
function render() {
  const st = stack[stack.length - 1];
  VIEWS.forEach(v => $('#view-' + v).classList.toggle('hidden', v !== st.view));
  $('#top-title').textContent = st.title || '公考助手';
...
function goHome() { stack = [{ view: 'home', title: '公考助手' }]; render(); }
...
async function init() {
  ...
  $('#home-cards').innerHTML = SECTIONS.map(s => `
    <div class="home-card" data-sec="${esc(s.key)}">
      <div class="hc-logo">${esc(s.icon)}</div>
      <div class="hc-name">${esc(s.name)}</div>
      <div class="hc-desc">${esc(s.desc)}</div>
      <div class="hc-count">${s.boards.length} 个板块</div>
    </div>`).join('');
  goHome();
}
...
function openSection(key) {
  ...
  $('#board-grid').dataset.sec = sec.name;
  push({ view: 'section', title: sec.name, key });
}

$('#board-grid').addEventListener('click', e => {
  const c = e.target.closest('[data-board]'); if (!c) return;
  openBoard($('#board-grid').dataset.sec, c.dataset.board);
});

function openBoard(section, board) {
  cur = { section, board };
  $('#board-title').textContent = board;
  const isIdiom = board === IDIOM_BOARD;
  $('#idiom-tool').classList.toggle('hidden', !isIdiom);
  push({ view: 'board', title: board });
  if (isIdiom) { state.page = 1; state.filter = 'all'; state.q = ''; resetIdiomUI(); loadEntries(); }
  loadMaterials();
}
...
function iconFor(ext) { return EXT_ICON[(ext || '').replace('.', '')] || '📎'; }

async function loadMaterials() {
  try {
    const d = await api('/api/materials?board=' + encodeURIComponent(cur.board));
    const box = $('#mat-list');
    if (!d.items.length) {
      box.innerHTML = ''; $('#mat-empty').classList.remove('hidden'); return;
    }
    ...
    box.innerHTML = d.items.map(m => `
      <div class="mat-item" data-id="${m.id}" data-view="${m.viewable ? 1 : 0}">
        ...
        <div class="mat-info">
          ...
          <div class="mat-meta">${esc((m.ext || '').replace('.', '').toUpperCase())} · ${fmtSize(m.size)}</div>
        </div>
...
$('#mat-list').addEventListener('click', async e => {
  ...
  if (act) {
    ...
    if (act.dataset.act === 'dl') {
      ...
      a.href = '/api/materials/' + id + '/download'; a.download = '';
...
$('#up-go').onclick = async () => {
  ...
  fd.append('file', file);
  fd.append('section', cur.section);
  fd.append('board', cur.board);
...
let preview = null;
function resetIdiomUI() {
  $('#word-input').value = ''; $('#preview').classList.add('hidden');
  $('#search').value = ''; preview = null;
...
async function doLookup() {
  ...
  try {
    ...
    $('#pv-exp').value = d.explanation; $('#pv-der').value = d.derivation;
    $('#pv-exa').value = d.example; $('#pv-note').value = ''; $('#pv-catsel').value = d.category;
  ...
  try {
    await api('/api/entries', {
      ...
      body: JSON.stringify({
        ...
        explanation: $('#pv-exp').value, derivation: $('#pv-der').value,
        example: $('#pv-exa').value, note: $('#pv-note').value,
      }),
    });
...
function goPage(p) {
  ...
  state.page = p; loadEntries(); window.scrollTo({ top: 0, behavior: 'smooth' });
}
$('#list').addEventListener('click', async e => {
  ...
  if (btn.dataset.act === 'star') {
    try { await api('/api/entries/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ starred: !it.starred }) }); loadEntries(); }
    catch (err) { toast(err.message, true); }
    ...
    try { await api('/api/entries/' + id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ note }) }); loadEntries(); }
    catch (err) { toast(err.message, true); }
  }
});
...
$('#search').addEventListener('input', e => {
  ...
  searchTimer = setTimeout(() => { state.q = e.target.value.trim(); state.page = 1; loadEntries(); }, 250);
});
...
$('#ex-go').onclick = async () => {
  ...
  if (IN_APP) {
    ...
    p.set('mode', body.mode); p.set('der', body.derivation ? 1 : 0);
    p.set('exa', body.example ? 1 : 0); p.set('note', body.note ? 1 : 0);
...
$('#set-save').onclick = async () => {
  ...
  try {
    await api('/api/account', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    toast('已保存'); $('#settings-modal').classList.add('hidden');
  } catch (e) { toast(e.message, true); }
};
