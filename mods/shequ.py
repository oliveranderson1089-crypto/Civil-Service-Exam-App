"""社区专职工作者（本县）：整卷背题 / 模考，以及**入库校对的裁决台**。
...
def overview():
    ...
    return jsonify({"papers": [dict(r) for r in rows], "mocks": mocks, "types": types,
                    ...
                    # 卷面结构和本地真题不一样（没有案例分析和公文写作那 40 分）。
                    "mock_note": "外省机构编的模拟卷，不是本地真题：卷面只有客观题，"
                                 "没有本地那 40 分的案例分析和公文写作。原件是扫描件，"
...
# ---------------------------------------------------------------- 主观题（40 分）
# 主观题的三档，**摆在一起但不混为一谈**：
#   real     本地两套原卷上的四道题 —— 这是标尺，永远排最前
#   offsite  外省真题里和本地同型的（案例分析 / 公文写作）
#   short    简答论述。本地卷面上**没有这个题型**，只当知识点自测用
# 分档写在这儿而不是前端：JS 里再判一遍迟早和后端说的不是一回事。
SUB_GROUPS = [
    ("real", "本地真题", "两套原卷上的原题。这是标尺 —— 别的题都照着它的形状练。"),
    ("offsite", "外省真题 · 同型", "别的省份考过的案例分析和公文写作，形状和本地一样。"
                                   "满分是照本地口径给的（案例 12 分、公文 15 分），"
                                   "外省原卷没标分值。"),
    ("short", "简答论述 · 本地未考", "外省题库带来的题型，本地两套原卷上没有。"
