...
# 卷面上那条**横线**：word 里它不是字符，是一串不断行空格（\xa0，常和普通空格交替）。
# 归一化会把它压成一个空格，于是「市场主体越要\xa0×8。」存成「市场主体越要 。」——
# 选词填空最要紧的信息（该往哪儿填、填几个空）就这么没了，题干看着像少打了两个字。
# 所以先把它换成看得见的横线再压空白。
#
# 怎么和**排版缩进**区分：缩进永远顶在行首（实测只有 2 个和 4 个两种长度，1581 处），
# 横线永远长在字的后面。所以只认「前面紧挨着一个实字」的空白串 —— 光看长度分不开，
# 行首的 4 连和句中的 4 连（「74、（\xa0\xa0\xa0\xa0）对于 红酒」）一样长。
# 至少 3 格：两格的多是词与词之间的排版间隙（「对于\xa0\xa0红酒」）。
# 必须含 \xa0：纯普通空格的间隔是排版（「A、①②③④   B、①③②④」四个选项挤一行）。
_BLANK_RUN = re.compile(r"(?<=\S)[\xa0 ]{3,}")
# 用全角下划线：qhash_text 会把 FF01~FF5E 转半角、再把 `_` 和空白一起当标点删掉，
# 所以**题干指纹一个字节都不变** —— 老库补上横线不会被判成另一道题。
_BLANK = "＿＿＿＿"


def _blank_or_gap(m):
    return _BLANK if "\xa0" in m.group(0) else m.group(0)


def norm(s):
    """统一空白与全角标点 —— 同一道题在 word 版和 PDF 版里空格数往往不一样，
       不归一化的话去重时会当成两道题。填空的横线先还原成 ＿＿＿＿ 再压。"""
    if not s:
        return ""
    s = _BLANK_RUN.sub(_blank_or_gap, s)
...
def parse_paper(text):
    ...
    """
    # 全角空格照旧压成半角；**\xa0 留着**交给 norm —— 在这儿抹平的话，选词填空的
    # 横线（就是一串 \xa0）到 norm 手上已经变成普通空格，和排版间隙再也分不开了。
    # 下面各条正则的 [\s　] 里 \s 本来就吃 \xa0，留着不影响切题。
    text = text.translate(_KANGXI_TAB).replace("　", " ")
...
# 分隔符要可有可无：2004 国考 B 卷排的是「1【答案】C」，题号后面**什么都没有**。
# 后面紧跟着「【答案】」这么强的锚，不要分隔符也不会误伤。
_ANS_INLINE = re.compile(r"^[\s　]*(\d{1,3})[\s　]*[、.．]?[\s　]*"
                         # 冒号那一格要放宽：扫描件里 ：常被认成 ，或 、
                         # （2022 四川那份实见「3、正确答案，D，」，只认冒号整卷剩 6 条）
...
# ⑬「【解析 9一正确答案A】」—— **题号和答案都在块头里**，比什么都可靠
#    （2024 国考地市卷/副省卷那批 A0 扫描件）。中间那个「一」是分隔符被 OCR
#    认花了（原本多半是 · 或 -），所以只说「中间隔着不超过 8 个非右括号字符」。
#    「解析」二字有时不写（首块常排成「【1】【正确答案B】」），所以可有可无；
#    答案字母前也可能被 OCR 塞进一两个杂字（实见「正确答案人A」），
#    所以允许中间隔几个**非 A-D** 的字符 —— 隔的字符若也可能是 A-D，就会抓错答案。
_ANS_HEADANS = re.compile(r"^[\s　]*[【\[][\s　]*(?:解析[\s　]*)?(\d{1,3})[^\n]{0,10}?"
                          r"正确答案[\s　]*[:：]?[^A-D\n]{0,3}([A-D])"
                          r"[\s　]*[】\]|1)}]?[\s　]*$", re.M)
_ANS_NUM = re.compile(r"^[\s　]*[【\[][\s　]*(?:解析|题目)[\s　]*(\d{1,3})[\s　]*[】\]|1)}]?[\s　]*$", re.M)
...
_ANS_TIGHT = re.compile(r"^[\s　]*(\d{1,3})[\s　]*[、.．][\s　]*([A-D])[\s　]*"
                        r"(?=[【\[［〔]?[\s　]*解析)", re.M)
# ⑫ 题号整个被括号包住、答案字母紧跟其后：「【1】D。 2×6=12…」（2007 四川）。
#    末尾那个标点是必需的 —— 不要的话「【1】D 项错误」这种正文引用也会被当成答案。
_ANS_NUMBOX = re.compile(r"^[\s　]*[【\[［][\s　]*(\d{1,3})[\s　]*[】\]］][\s　]*"
                         r"([A-D])[\s　]*(?=[。.．，,、；;])", re.M)
...
def parse_answers(text):
    ...
    # ⑬ 题号和答案都印在块头里 —— 不用去正文里猜，最可靠，所以排在前面
    h = list(_ANS_HEADANS.finditer(text))
    if h:
        cands.append((_seg_answers(text, h, lambda m, b: m.group(2)), False))

    h = list(_ANS_NUMBOX.finditer(text))
    if h:
        cands.append((_seg_answers(text, h, lambda m, b: m.group(2)), False))
